...
from app.schemas.course import CourseCreate, CourseOut, CourseUpdate, CourseUpdateOut, PaginatedCourses
...
async def list_courses(
    current_user=Depends(get_current_user),
    skip: int = Query(
        0, description="Number of courses to skip for pagination"),
...
async def update_course(
    ...
    if existing_course["instructor"] != user["email"]:
        raise HTTPException(
            status_code=403,
            detail="you are not the creator of this course. so, You do not have permission to update this course"
        )
...
async def get_courses_by_category(
    ...
    if not courses:
        raise HTTPException(
            status_code=404, detail="No courses found in this category")
    return courses
